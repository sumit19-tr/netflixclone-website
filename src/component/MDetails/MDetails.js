    ...
    const renderMDetails = (data) => {
        if (data) {
            return data.map((item) => {
                return (
                    <>
                        <div className="Movie-banner" key={item._id}>
                            <img src={item.banner} alt="Movie-poster" className="Responsive-img col-12"
                                width="auto" height="400" />
                            <div className="col-6- col-sm-8 col-md-8 col-lg-4 bottom-left">
                                ...
                                <div className="detail-btn ">
                                    <Link to={`/play?movie_name=${item.title}`}>
                                        <button className="Movie-banner-btn mt-2" > <i className="fa fa-play mbtn"></i>Play</button>
                                    </Link>
